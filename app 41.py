...
def rank_candidates(df_pending: pd.DataFrame, schema: dict, model, top_k: int = 10) -> pd.DataFrame:
    ...
    X_input = df_aligned.reindex(columns=expected, fill_value=0)  # preencher colunas faltantes com 0
    ...
    df_aligned["rank"] = df_aligned.groupby(schema["id_vaga_col"])[
        "score"
...
if uploaded is not None:
    ...
    if "inf_titulo_vaga" in ranking.columns:
        ranking["vaga_display"] = (
            ranking[schema["id_vaga_col"]].astype(str)
            + " - "
            + ranking["inf_titulo_vaga"].astype(str)
        )
    else:
    ...
    with st.expander("📊 Detalhes completos da vaga e candidatos"):
        cols_show = [
            schema["id_vaga_col"], "inf_titulo_vaga", "inf_cliente",
